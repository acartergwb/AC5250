...
/// <summary>
/// EBCDIC (CCSID 37) to/from ASCII translation.
/// </summary>
public static class Ebcdic
{
    private static readonly byte[] EbcdicToAsciiTable = new byte[256]
    {
        // 0x00-0x0F
    ...
    {
        // Build reverse table
        Array.Fill(AsciiToEbcdicTable, (byte)0x40); // default to EBCDIC space
        for (int i = 0; i < 256; i++)
        {
            byte ascii = EbcdicToAsciiTable[i];
            AsciiToEbcdicTable[ascii] = (byte)i;
        }
    }

    public static char ToAscii(byte ebcdic)
    {
        return (char)EbcdicToAsciiTable[ebcdic];
    }

    public static byte FromAscii(char ascii)
    {
        if (ascii > 255) return 0x40; // space for unmappable
        return AsciiToEbcdicTable[(byte)ascii];
    }
